@extends('layouts.app')
@section('title', 'Sales by Cashier - VetPOS')
@section('content')
<h1 class="text-2xl font-bold mb-6">Sales by Cashier</h1>
...
<div class="bg-white rounded-lg border border-gray-200 overflow-x-auto">
    <table class="w-full text-sm">
        <thead class="bg-gray-50"><tr><th class="text-left px-4 py-2">Cashier</th><th class="text-right px-4 py-2">Transactions</th><th class="text-right px-4 py-2">Total Sales</th></tr></thead>
        <tbody class="divide-y divide-gray-100">
            @forelse($data as $row)
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-2 font-medium">{{ $row->user->name ?? 'Deleted' }}</td>
                <td class="px-4 py-2 text-right">{{ $row->transaction_count }}</td>
