...
<div class="max-w-2xl">
    ...
    <form method="POST" action="{{ route('admin.owners.update', $owner) }}" enctype="multipart/form-data" class="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
        @csrf @method('PUT')
        <div>
        ...
        <div>
            ...
            <div class="flex items-center gap-4">
                <div id="photo-preview" class="w-16 h-16 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400 text-xs overflow-hidden bg-gray-50 flex-shrink-0">
                    <span id="photo-placeholder" class="{{ $owner->photo_url ? 'hidden' : '' }}">No photo</span>
                    <img id="photo-thumb" class="w-full h-full object-cover {{ $owner->photo_url ? '' : 'hidden' }}" src="{{ $owner->photo_url ?? '' }}" alt="">
                </div>
                <div class="flex-1">
